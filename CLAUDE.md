...
- `line-crossing` — Line-crossing counting (`LineCrossingCounter`): tallies tracked points crossing anchor-defined lines, with optional ID-churn assist (rescue / cooldown). Detector- and tracker-agnostic (consumes `{ trackId, point }` + `Line = {id, p1, p2, forwardDirection?}`); drawing is out of scope for this package. `forwardDirection` (a `Vector`, or an ordered `Vector[]` whose first entry not parallel to the line wins) fixes which side counts as `forward`, independently of endpoint order; it defaults to `DEFAULT_FORWARD_DIRECTION` (rightward, downward for a horizontal line). `forwardNormal(line)` exposes the resolved side as a unit normal so callers can draw it without reimplementing the rule, and `reverseDirection(directions?)` returns the opposite policy for a per-line flip toggle. Ported from sandbox `src/lib/lines/`, renamed to make the purpose explicit (the subpath is `web-crowd-detection-utils/line-crossing`).
...
- **pnpm built dependencies:** `pnpm-workspace.yaml#allowBuilds` gates which postinstall scripts are allowed (a `name: true|false` map). `esbuild` and `protobufjs` are approved (transitive deps). Add an entry there rather than running `pnpm approve-builds` interactively. This setting moved out of `package.json#pnpm.onlyBuiltDependencies`, which pnpm v11 removed — do not re-add the `pnpm` field to `package.json`.
